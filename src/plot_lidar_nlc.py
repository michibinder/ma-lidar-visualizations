...
def plot_lidar_nlc(config, obs, pbar):
    print(obs) 
    """Visualize lidar measurement (nlc)"""
    file_name = os.path.split(obs)[-1]
    print(file_name)
    zrange = eval(config.get("GENERAL","ALTITUDE_RANGE_NLC"))
    print(zrange)
    ...
    # trying to remove insignificant values
    # np.nan where condition is true, and ds.bsr elsewhere
    # sigma = 2.5
    ds.bsr.values = np.where(ds.bsr.values < 1*ds.bsr_err.values, np.nan, ds.bsr.values)
    ...
    filter_str = "Volume backscatter coefficient"
    ...
    cbar_l    = "volume backscatter coefficient" 
    cbar_l    = "" 
...
#---
    ...
    axes.set_ylim(zrange[0],zrange[1])

    """Formatting"""
    TRES = int(config.get("GENERAL","RESOLUTION_NLC").split("Z")[0][2:])
    VRES = int(config.get("GENERAL","RESOLUTION_NLC").split("Z")[-1][:-3])
    
    axes.text(1, -0.75, np.char.add('date created ',ds['date_created'].to_numpy()), horizontalalignment='right',verticalalignment='bottom',transform=axes.transAxes, fontsize=10)
    ...
    {}, {}\n \
    ------------------------------\n \
    Resolution: {}$\,$m x $\,$ 10 min'.format(config.get("GENERAL","INSTRUMENT"), config.get("GENERAL","STATION_NAME"), VRES))
